<template>
  <div class="columns">
    ...
    <div class="column col-12">
      ...
      <p>Customize MEI to your specific needs with an ODD customization. At this point, customizations will always be  based on MEI v4.0.1.</p>
    </div>
    <div class="optionsBox column col-2">
      <div class="stickyBox">
        ...
        <div class="displayOptions">
          <div class="showDesc"><span class="form-group customSwitch"><label class="form-checkbox form-inline input-sm"><input v-on:click="toggleDesc()" type="checkbox" v-bind:checked="showDesc"><i class="form-icon"></i></label></span> show desc</div>
          <div class="showInactive"><span class="form-group customSwitch"><label class="form-checkbox form-inline input-sm"><input v-on:click="toggleInactiveDisplay()" type="checkbox" v-bind:checked="showInactive"><i class="form-icon"></i></label></span> show inactive</div>
        </div>
      </div>
    </div>
    <div class="profileDrafter column col-10" v-bind:class="{'hideDesc': !showDesc, 'hideInactive': !showInactive}">
      <div class="columns" style="margin-left: .5rem;">
        <div v-if="level === 'modules'" class="column col-12" style="margin-top: 0;">
          <div v-for="module in modules">
            <ProfilerModule v-bind:module="module"/>
          </div>
        </div>
        <div v-if="level === 'singleModule'" class="column col-12" style="margin-top: 0;">
          <div class="heading module stickyBox">
            <div>
              <button class="btn btn-link" title="show modules" v-on:click="showModuleSelection()"><i class="icon icon-back"></i></button>
              <span class="moduleName">{{currentModule.name}}</span>
            </div>
            <div class="desc">
              {{currentModule.desc}} (elements:&nbsp;<span class="number">{{currentModule.elementCount}}</span>, attribute&nbsp;classes:&nbsp;<span class="number">{{currentModule.attClassCount}}</span>)</div>
          </div>
        ...
        <div v-if="level === 'element'" class="column col-12" style="margin-top: 0;">
          <div class="heading element stickyBox">
            <div>
              <button class="btn btn-link" title="show modules" v-on:click="goBack()"><i class="icon icon-back"></i></button>
              <span class="elementName">&lt;{{currentElement.name}}&gt;</span>
            </div>
            <div class="desc">
              {{currentElement.desc}}</div>
          </div>
          <ProfilerAttribute v-for="att in currentAtts.atts" v-bind:att="att"/>
          <ProfilerAttClass v-for="attClass in currentAtts.classes" v-bind:attClass="attClass"/>
        </div>
      </div>
    </div>
  </div>
...
.profileDrafter {
  ...
  &.hideInactive .inactive, &.hideInactive .inactiveModule {
    display: none;
  }
