<template>
  <div class="attClass" v-bind:title="attClass.name" v-bind:class="{'inactive': inactive, 'inactiveModule':inactiveModule}">
    <span class="form-group customSwitch"><label class="form-checkbox form-inline input-sm"><input v-on:click="toggleAttClass()" type="checkbox" v-bind:checked="active"><i class="form-icon"></i></label></span>
    ...
    <div v-if="hasChildClasses">
      <ProfilerAttClass v-for="attClass in attClass.classes" v-bind:attClass="attClass"/>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    inactive: function() {
      return this.$store.getters.profilerInactiveAttClasses.indexOf(this.attClass.name) !== -1
    },
    inactiveModule: function() {
      if(typeof this.attClass.module === 'undefined') {
        return false;
      }
      return this.$store.getters.profilerInactiveModules.indexOf(this.attClass.module) !== -1
    },
    active: function() {
      return this.$store.getters.profilerInactiveAttClasses.indexOf(this.attClass.name) === -1
    },
    hasAttributes: function() {
      return (typeof this.attClass.atts !== 'undefined' && this.attClass.atts.length > 0)
    },
    hasChildClasses: function() {
      return (typeof this.attClass.classes !== 'undefined' && this.attClass.classes.length > 0)
    },
    moduleDesc: function() {
      if(typeof this.attClass.module === 'undefined') {
        return '';
