...
export default {
  ...
  methods: {
    ...
    setSchemaSpec: function () {
      ...
      //console.log(oddJson);
      let tei = oddJson.elements.filter((elem) => {
        return elem.type === "element" && elem.name === "TEI";
      })[0];
      let text = tei.elements.filter((elem) => {
        return elem.type === "element" && elem.name === "text";
      })[0];
      let body = text.elements.filter((elem) => {
        return elem.type === "element" && elem.name === "body";
      })[0];
      schemaSpec = body.elements.filter((elem) => {
        return elem.type === "element" && elem.name === "schemaSpec";
      })[0];
    },
    parseExistingCustomization: function () {
      ...
      // adjust modules
      let moduleRefs = schemaSpec.elements.filter((elem) => {
        return elem.type === "element" && elem.name === "moduleRef";
      });
      ...
      let removedClassSpecs = schemaSpec.elements.filter((elem) => {
        return (
          elem.type === "element" &&
          elem.name === "classSpec" &&
          elem.attributes.type === "atts" &&
          elem.attributes.mode === "delete"
        );
      });
    ...
    removeModule: function (ident) {
      ...
      schemaSpec.elements.forEach((moduleRef, i) => {
        if (
          moduleRef.type === "element" &&
          moduleRef.name === "moduleRef" &&
          moduleRef.attributes.key === ident
        ) {
          index = i;
        }
      });
    ...
    trackChangeInOdd: function () {
      ...
      let tei = customization.elements.filter((elem) => {
        return elem.type === "element" && elem.name === "TEI";
      })[0];
      let teiHeader = tei.elements.filter((elem) => {
        return elem.type === "element" && elem.name === "teiHeader";
      })[0];
      let revisionDesc = teiHeader.elements.filter((elem) => {
        return elem.type === "element" && elem.name === "revisionDesc";
      })[0];
      ...
      if (typeof revisionDesc === "undefined") {
        ...
        let changes = revisionDesc.elements.filter((elem) => {
          return elem.type === "element" && elem.name === "change";
        });
        if (changes.length === 0) {
            ...
            elements: [
              {
                type: "text",
                text:
                  "ODD " + verb + " online with https://meigarage.edirom.de",
              },
            ],
          },
        ],
  ...
  mounted() {
    unwatch = this.$store.watch(
      (state, getters) => ({
        maintenanceMode: state.profiler.maintenanceMode,
        customization: getters.profilerActiveCustomization,
        inactiveModules: getters.profilerInactiveModules,
        inactiveAttClasses: getters.profilerInactiveAttClasses,
        inactiveElements: getters.profilerInactiveElements,
      }),
      (newState, oldState) => {
        //make sure to switch to custom customization if needed
        if (
          newState.customization !== oldState.customization &&
          newState.customization === "custom"
        ) {
        ...
        // modules modified
        if (
          newState.inactiveModules.length !== oldState.inactiveModules.length
        ) {
          // module removed
          if (
            newState.inactiveModules.length > oldState.inactiveModules.length
          ) {
            let ident = newState.inactiveModules.filter((mod) => {
              return oldState.inactiveModules.indexOf(mod) === -1;
            })[0];
            this.removeModule(ident);
            // module added
          } else {
            let ident = oldState.inactiveModules.filter((mod) => {
              return newState.inactiveModules.indexOf(mod) === -1;
            })[0];
        ...
        // elements modified
        if (
          newState.inactiveElements.length !== oldState.inactiveElements.length
        ) {
          // element removed
          if (
            newState.inactiveElements.length > oldState.inactiveElements.length
          ) {
            let ident = newState.inactiveElements.filter((mod) => {
              return oldState.inactiveElements.indexOf(mod) === -1;
            })[0];
            this.removeElement(ident);
            // element added
          } else {
            let ident = oldState.inactiveElements.filter((mod) => {
              return newState.inactiveElements.indexOf(mod) === -1;
            })[0];
        ...
        if (
          newState.inactiveAttClasses.length !==
          oldState.inactiveAttClasses.length
        ) {
          // att class removed
          if (
            newState.inactiveAttClasses.length >
            ...
            let ident = newState.inactiveAttClasses.filter((mod) => {
              return oldState.inactiveAttClasses.indexOf(mod) === -1;
            })[0];
            this.removeAttClass(ident);
            // att class added
          } else {
            let ident = oldState.inactiveAttClasses.filter((mod) => {
              return newState.inactiveAttClasses.indexOf(mod) === -1;
            })[0];
